...
{
  asd_node_t *ret = NULL;
  ret = calloc(1, sizeof(asd_node_t));
  if (ret != NULL){
...
void asd_add_child(asd_node_t *tree, asd_node_t *child)
{
  if (tree != NULL && child != NULL){
    ...
    tree->children = realloc(tree->children, tree->number_of_children * sizeof(asd_node_t*));
...
static void _asd_print (FILE *foutput, asd_node_t *tree, int profundidade)
{
  int i;
  if (tree != NULL){
...
static void _asd_print_graphviz (FILE *foutput, asd_node_t *tree)
{
  int i;
  if (tree != NULL){
