...
cabecalho: TK_IDENTIFICADOR parametros TK_OC_MAP tipo       { $$ = ast_new($1->token_value); $$ = ast_new(TK_OC_MAP); ast_add_child($$, $2); ast_add_child($$, $2); }//TODO: revisar
...
var_local: TK_IDENTIFICADOR                                 { $$ = ast_new($1->token_value); }
    | TK_IDENTIFICADOR TK_OC_LE literal                     { $$ = ast_new($1->token_value); $$ = ast_new(TK_OC_LE); ast_add_child($$, $3);}
...
op_retorno: TK_PR_RETURN expressao                          { $$ = ast_new(TK_PR_RETURN); ast_add_child($$, $2); }

fluxo_ctrl: condicional                                     { $$ = $1; }
    | interativa                                            { $$ = $1; }

condicional: TK_PR_IF '(' expressao ')' bloco_cmd TK_PR_ELSE bloco_cmd      { $$ = ast_new(TK_PR_IF); ast_add_child($$, $3); ast_add_child($$, $5); $$ = ast_new(TK_PR_ELSE); ast_add_child($$, $7); }
    | TK_PR_IF '(' expressao ')' bloco_cmd                                  { $$ = ast_new(TK_PR_IF); ast_add_child($$, $3); ast_add_child($$, $5); }

interativa: TK_PR_WHILE '(' expressao ')' bloco_cmd         { $$ = ast_new(TK_PR_WHILE); ast_add_child($$, $3); ast_add_child($$, $5); }//TODO: bloco não deve ser incluido?
...
op_pre_4: '<'                                               { $$ = ast_new("<"); } 
    | '>'                                                   { $$ = ast_new(">"); }   
    | TK_OC_LE                                              { $$ = ast_new(TK_OC_LE); }
    | TK_OC_GE                                              { $$ = ast_new(TK_OC_GE); }   

op_pre_5: TK_OC_EQ                                          { $$ = ast_new(TK_OC_EQ); }
    | TK_OC_NE                                              { $$ = ast_new(TK_OC_NE); } 

op_pre_6: TK_OC_AND                                         { $$ = ast_new(TK_OC_AND); }  

op_pre_7: TK_OC_OR                                          { $$ = ast_new(TK_OC_OR); } 
...
tipo: TK_PR_INT                                             { $$ = ast_new(TK_PR_INT); }                                           
    | TK_PR_FLOAT                                           { $$ = ast_new(TK_PR_FLOAT); }                                          
    | TK_PR_BOOL                                            { $$ = ast_new(TK_PR_BOOL); }                                          
